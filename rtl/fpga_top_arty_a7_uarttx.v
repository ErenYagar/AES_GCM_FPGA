...
module fpga_top_arty_a7_uarttx (
    input  wire       clk,
    input  wire       rst_btn,
    output wire [3:0] led,
...
localparam integer MSG_BYTES = 6;
...
begin
    if (pass_flag) begin
        case (idx)
            ...
            3'd2: get_msg_byte = "S";
            3'd3: get_msg_byte = "S";
            3'd4: get_msg_byte = 8'h0d;
            default: get_msg_byte = 8'h0a;
        endcase
    end
    else begin
        case (idx)
...
always @(posedge core_clk) begin
    if (rst) begin
        ...
        if ((pass_latched || fail_latched) && !msg_started && !tx_busy) begin
            tx_data_reg  <= get_msg_byte(pass_latched, 3'd0);
            tx_start_reg <= 1'b1;
            msg_started  <= 1'b1;
            msg_idx      <= 3'd1;
        end
        else if (msg_started && tx_done && (msg_idx < MSG_BYTES[2:0])) begin
            tx_data_reg  <= get_msg_byte(pass_latched, msg_idx);
            tx_start_reg <= 1'b1;
