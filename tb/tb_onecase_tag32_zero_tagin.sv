`timescale 1ns / 1ps
module tb_onecase_tag32_zero_tagin;
...
initial begin
    ...
    while(wait_cycles<40000) begin
        ...
        if((ct_seen==13) && (tag_seen==4)) begin
            $display("One-case tag32 with zero tag input completed.");
            #20;
            $finish;
        end
    end
    $fatal(1,"Timeout waiting for one-case tag32 zero-tag-input regression.");
end
endmodule
