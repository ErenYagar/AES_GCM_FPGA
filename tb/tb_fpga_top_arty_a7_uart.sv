...
module tb_fpga_top_arty_a7_uart;
    ...
    localparam integer UART_WAIT_TIMEOUT_NS = 20000000;
    ...
    begin
        ...
        if (!got_start) begin
            $display("DBG eng_state=%0d run_request=%0b result_valid=%0b status=0x%02x frame_active=%0b tx_busy=%0b send_status=%0b send_pc=%0b send_tag=%0b pc_len=%0d tag_cnt=%0d",
                     dut.eng_state, dut.run_request, dut.result_valid, dut.status_code_reg,
                     dut.frame_active, dut.tx_busy, dut.send_status_pending, dut.send_pc_pending,
                     dut.send_tag_pending, dut.pc_capture_len_bits, dut.tag_capture_count);
    ...
    initial begin
        ...
        host_send_packet(CMD_AAD,  8,   1,  {1016'd0, 8'h01});
        host_send_packet(CMD_PT,   8,   1,  {1016'd0, 8'h02});
        ...
        if (frame_type !== RSP_STATUS)
            $fatal(1, "Expected STATUS frame, got type=0x%02x", frame_type);
        if (bit_len !== 8)
            $fatal(1, "STATUS bit_len mismatch: %0d", bit_len);
        recv_byte_with_timeout(payload_byte);
        if (payload_byte !== 8'h00)
            $fatal(1, "STATUS payload mismatch: 0x%02x", payload_byte);

        recv_frame_header(frame_type, bit_len, payload_bytes);
        if (frame_type !== RSP_PC)
            $fatal(1, "Expected PC frame, got type=0x%02x", frame_type);
        if (bit_len !== 8)
            $fatal(1, "PC bit_len mismatch: %0d", bit_len);
        if (payload_bytes !== 1)
            $fatal(1, "PC payload byte count mismatch: %0d", payload_bytes);
        recv_byte_with_timeout(payload_byte);

        recv_frame_header(frame_type, bit_len, payload_bytes);
        if (frame_type !== RSP_TAG)
            $fatal(1, "Expected TAG frame, got type=0x%02x", frame_type);
        if (bit_len !== 128)
            $fatal(1, "TAG bit_len mismatch: %0d", bit_len);
        if (payload_bytes !== 16)
            $fatal(1, "TAG payload byte count mismatch: %0d", payload_bytes);
        for (i = 0; i < 16; i = i + 1)
            recv_byte_with_timeout(payload_byte);

        $display("UART protocol smoke completed.");
